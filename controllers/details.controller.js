...
exports.render = async (req, res) => {
    ...
    const updatedLocations = await Promise.all(
        locations.map(async (location) => {
            ...
            const localCovidDataSeries = counties.reduce((accumulator, current) => {
                return [...accumulator, { id: current.countyName, values: current.historicData.sort((a, b) => a.date.localeCompare(b.date)) }]
            }, [])

            return { ...location, hereData: hereData, localCovidData: localCovidDataSeries, statesInfo: statesInfo.find(({ state }) => state === location.state_abbr), }
        })
    )
